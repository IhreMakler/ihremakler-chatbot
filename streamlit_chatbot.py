...
# Chatverlauf und Eingabefeld direkt unter dem Header
...
<div style="display: flex; gap: 1rem; align-items: center; margin-bottom: 2rem;">
  <input type="text" id="chat_input" name="chat_input" placeholder="Stellen Sie eine Frage..." style="flex: 1; padding: 0.8rem; border: 1px solid #3B8EA0; border-radius: 4px; font-family: Arial, Verdana, sans-serif; font-size: 1rem;" onkeydown="if(event.key==='Enter'){document.getElementById('send_btn').click();}">
  <button id="send_btn" style="background-color: #3B8EA0; color: white; border: none; padding: 0.8rem 1.5rem; border-radius: 4px; cursor: pointer; font-weight: 600; font-family: Arial, Verdana, sans-serif; font-size: 1rem;">Senden</button>
</div>
<script>
  const input = document.getElementById('chat_input');
  const btn = document.getElementById('send_btn');
  btn.onclick = function() {
    window.dispatchEvent(new Event('streamlit_send'));
  };
</script>
""", unsafe_allow_html=True)

user_input = st.text_input("Ihre Frage:", placeholder="Stellen Sie eine Frage...", key="chat_input_streamlit")
send_button = st.button("Senden", use_container_width=True, key="send_btn_streamlit")
