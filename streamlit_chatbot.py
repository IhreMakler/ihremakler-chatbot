...
# Eingabefeld und Button nebeneinander, bündig und auf gleicher Höhe
st.markdown("""
<div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 2rem;">
  <input id="chat_input_html" type="text" placeholder="Stellen Sie eine Frage..." style="flex: 1; padding: 0.8rem; border: 1px solid #3B8EA0; border-radius: 4px; font-family: Arial, Verdana, sans-serif; font-size: 1rem; height: 48px;">
  <button id="send_btn_html" style="background-color: #3B8EA0; color: white; border: none; border-radius: 4px; font-weight: 600; font-family: Arial, Verdana, sans-serif; font-size: 1rem; height: 48px; padding: 0 2rem;">Senden</button>
</div>
<script>
  const input = document.getElementById('chat_input_html');
  const btn = document.getElementById('send_btn_html');
  input.addEventListener('keydown', function(e) {
    if (e.key === 'Enter') btn.click();
  });
  btn.onclick = function() {
    window.parent.postMessage({isStreamlitMessage: true, type: 'streamlit:setComponentValue', value: input.value, key: 'chat_input'}, '*');
    input.value = '';
  };
</script>
""", unsafe_allow_html=True)
...
if "chat_input" in st.session_state and st.session_state["chat_input"]:
    ...
    components.html("""
    <script>
    document.querySelector('input[placeholder="Stellen Sie eine Frage..."]').addEventListener('keydown', function(e) {
        if (e.key === 'Enter') {
            window.parent.postMessage({isStreamlitMessage: true, type: 'streamlit:setComponentValue', value: true, key: 'enter_sent'}, '*');
        }
    });
    </script>
    """, height=0)
